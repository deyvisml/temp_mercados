...
<script type="text/javascript">
    function vrepolice_ver_datos(){
        ...
        $.post("<?php echo site_url("modulo1/get_datos_tramite"); ?>",param,function(data){
			...
			$("#vrepolice_btn_reimprimir").css("display","block");
        });
    }
    $("#vrepolice_form_busca").submit(function(e){
        ...
        obsetec=$("#vrepolice_tra_obs_tec").val();
        $("#vrepolice_frame_pdf").attr("src","<?php echo site_url("modulo1/pdf_licencia"); ?>"+"/"+tra_ide+"?expe="+expe+"&obse="+obse+"&obsetec="+obsetec);
        $("#vrepolice_dialog_pdf").modal("show");
    });
	$("#vrepolice_btn_reimprimir").click(function(){
		tra_ide=$("#vrepolice_tra_ide").val();        
        $("#vrepolice_frame_pdf").attr("src","<?php echo site_url("modulo1/pdf_licencia"); ?>"+"/"+tra_ide+"/reimprimir");
        $("#vrepolice_dialog_pdf").modal("show");
	});
        ...
        <form id="vrepolice_form_busca">
	        ...
	        <div class="row" id="vrepolice_div_llena_datos" style="display:none;">
                ...
                <div class="col-sm-10">
                    <div class="form-group fg-float" style="display:block;">
                        <div class="fg-line">
                            <input type="text" class="input-sm form-control fg-input" id="vrepolice_tra_obs_tec" required value="-">
                        </div>
                        <label class="fg-label">Observaciones del tecnico</label>
                    </div>
                </div>
			</div>
            <div class="row">
            	<div class="col-sm-12">
					...
					<br>
					<button class="btn btn-success waves-effect" type="button" id="vrepolice_btn_reimprimir" style="display:none;">Reimprimir</button>
                </div>
            </div>
        </form>
    </div>
